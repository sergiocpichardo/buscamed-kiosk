...
export default class App extends Component {
  ...
  getDoctors = specialityId => {
    ...
    this.setState(
      ...
      () => {
        api.center
          .getDoctorsBySpeciality(centerId, userType, specialityId)
          .then(response => {
            this.setState({
              doctorListLoading: false,
  ...
  makeAppointment = () => {
    this.setState(
      ...
      () => {
        api.appointment
          .saveAndCreatePatient(this.state)
          .then(response => {
            this.setState(
              {
  ...
  loadSchedule = (doctor, date) => {
    this.setState(
      ...
      () => {
        ...
        api.doctor
          .getSchedule(doctor.id, centerId, centerType, date.dateString)
          .then(response => {
  ...
  showCalendar = async (key, doctor) => {
    try {
      ...
      if (action !== DatePickerAndroid.dismissedAction) {
        const date = moment(`${year}-${month + 1}-${day}`, 'YYYY-M-D').format(
          'YYYY-MM-DD'
        )
        const dayName = moment(date, 'YYYY-MM-DD')
          .format('ddd')
          .toLowerCase()
        ...
        if (
          workingHours.hasOwnProperty(dayName) &&
          ...
          this.setState(
            ...
            () => {
              const { centerId, centerType } = this.state
              api.doctor
                .getSchedule(doctor.id, centerId, centerType, date)
                .then(response => {
                  // alert(JSON.stringify(response));
                  this.setState(
                    {
  ...
  render () {
    ...
    const propsObject = {
      Home: { data: this.state.center, onNextSlide: this.handleNextSlide, path:this.path },
      IdInput: {
        handleReset: this.handleResetForm,
        loading: this.state.idCheckLoading,
        value: this.state.id,
      ...
      return (
        ...
        >
          <Text style={{ fontSize: 35, textAlign: 'center', marginBottom: 30 }}>
            Error al configurar el dispositivo, por favor revise su conexión a
            internet y que el dispositivo esté asociado a su centro{' '}
          </Text>
            ...
            {/* </TouchableOpacity> */}
              ...
              >
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.button,
                      { marginRight: 30, backgroundColor: '#A3A3A3' }
                    ]}
                  >
